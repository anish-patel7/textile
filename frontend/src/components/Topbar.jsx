...
export default function Topbar() {
  const { page, search, setSearch, dark, setDark, openNew } = useApp();

  return (
    <header style={{ height: 62, flexShrink: 0, background: 'var(--bg-panel)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14, padding: '0 22px' }}>
      <div style={{ fontWeight: 700, fontSize: 16, letterSpacing: '-.2px', whiteSpace: 'nowrap' }}>
        {pageTitles[page] || 'Textile Manager'}
      </div>

      <div style={{ flex: 1, maxWidth: 420, marginLeft: 8, position: 'relative' }}>
        ...
        <input
          ...
          placeholder="Search design no, DN, code, reed, color, old no…"
        />
      </div>
      ...
      <button
        onClick={() => setDark(d => !d)}
        title="Toggle theme"
        style={{ width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border)', borderRadius: 9, background: 'var(--bg-subtle)', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 16 }}
      >
        {dark ? '☀️' : '🌙'}
      </button>
