...
export default function Designs() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      setLoading(true);
      getDesigns(search, PAGE_SIZE, 0).then(r => {
  ...
  function loadMore() {
    ...
    setOffset(next);
    getDesigns(search, PAGE_SIZE, next).then(r => {
      const list = r.designs || [];
      setDesigns(prev => [...prev, ...list]);
      setHasMore(list.length === PAGE_SIZE);
    });
  }
    ...
    <div className="fade-in" style={{ padding: '24px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
        <div style={{ color: 'var(--text-muted)', fontSize: 13, fontWeight: 500 }}>
          {loading ? 'Loading…' : `${designs.length}${hasMore ? '+' : ''} designs`}
        </div>
        <div style={{ flex: 1 }} />
      ...
      {!loading && viewMode === 'grid' && designs.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))', gap: 16 }}>
          {designs.map(d => {
            ...
            return (
              ...
              >
                <div style={{ height: 140, background: 'var(--bg-inset)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  {img
                    ? <img src={img} alt={d.design_name} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : <svg width="36" height="36" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" stroke="var(--border-strong)" strokeWidth="1.3"/><circle cx="8.5" cy="8.5" r="1.5" stroke="var(--border-strong)" strokeWidth="1.3"/><path d="M21 15l-5-5L5 21" stroke="var(--border-strong)" strokeWidth="1.3" strokeLinejoin="round"/></svg>}
      ...
      {!loading && viewMode === 'table' && designs.length > 0 && (
      ...
      {hasMore && !loading && (
        <div style={{ textAlign: 'center', marginTop: 24 }}>
          <button className="btn-ghost" onClick={loadMore}>Load more designs</button>
        </div>
      )}
    </div>
  );
